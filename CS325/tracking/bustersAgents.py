...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        xy, gprob = None, 0
        for i, location in enumerate(localMax):
            if livingGhostPositionDistributions[i][location] >= gprob:
                xy, gprob = location, livingGhostPositionDistributions[i][location]
        temp = []
        for action in legal:
            nextLocation = Actions.getSuccessor(pacmanPosition, action)
            temp.append((self.distancer.getDistance(nextLocation, xy), action))
        return min(temp)[1]