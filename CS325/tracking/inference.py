...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in legalPos:  # all p in legal positions

            oldPos = p  # to use in position distribution
            newPos = self.setGhostPosition(gameState, oldPos)
            newPosDist = self.getPositionDistribution(newPos)

            for newPos, prob in newPosDist.items():
                allPossible[newPos] += prob * oldBeliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            goToJail = self.getJailPosition()
            numPart = self.numParticles
            self.particles = [goToJail for i in range(numPart)]  # all go to jail case
        else:
            ...
            for belief in beliefs:
                ...
                beliefs[belief] *= emissionModel[dist]  # multiplying new weight
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            newPart = util.sample(newPosDist)
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.particles: # each particle, weight, then normalize
            distrib[p] += 1
        distrib.normalize()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        allPossPos = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(allPossPos)  # shuffle the possible ones
        numPart = self.numParticles
        count, self.particles = 0, []
        while count < numPart:
            for position in allPossPos:
                if count < numPart:
                    self.particles.append(position)
                    count += 1
                else:
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for i, dist in enumerate(noisyDistances):
            if dist == None:
                capturedGhosts.append(i)

        for part in self.particles:
            for ghostIndex in capturedGhosts:
                part = self.getParticleWithGhostInJail(part, ghostIndex)
            prob = 1.0
            for i in range(self.numGhosts):
                if i not in capturedGhosts:
                    distance = util.manhattanDistance(part[i], pacmanPosition)
                    prob *= emissionModels[i][distance]
            partCount[part] += prob

        self.beliefs = partCount

        if partCount.totalCount() == 0:  # when all receive 0 weight
            self.initializeParticles()
        else:
            self.beliefs.normalize()
            temp = []
            for particle in self.particles:
                temp.append(util.sample(partCount))
            self.particles = temp
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            oldGPos = list(oldParticle)
            for i in range(self.numGhosts):
                newDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldGPos), i,self.ghostAgents[i])
